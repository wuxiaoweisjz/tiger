...
//      new Todo();
    ...
    private int maxArgsExpList(ExpList.T t) {
        if (null == t) {
            return 0;
        }
        if (t instanceof ExpList.Pair) {
            ExpList.Pair p = (ExpList.Pair) t;
            if (p.list instanceof ExpList.Pair) {
                return maxArgsExpList(p.list);
            } else if (p.list instanceof ExpList.Last) {
                return 2;
            }

        } else if (t instanceof ExpList.Last) {
            return 2;
        }
